         ...
         | roundAddr == 0xF00                     ->
            ...
            roundAddr
             ...
             | roundAddr == 0x00 ->
                case addr .&. 0xF of
                roundAddr
                 | roundAddr == 0              -> return $ Left $ "Read byte: " ++ show addr ++ " is undefined. Need to fix. "  {- TODO -}
                 | any (==roundAddr) [4,5,6,7] -> return $ Left $ "Read byte: " ++ show addr ++ " is undefined. Need to fix. "  {- TODO -}
                 | roundAddr == 15             -> gets $ \z80 -> Right $ z80^.mmu.iflag
                 | otherwise                   -> return $ Right 0
             | any (==roundAddr) [0x10,0x20,0x30]  -> return $ Right 0
             | any (==roundAddr) [0x40,0x50..0x70] -> return $ Right 0 {- TODO READ FROM GPU -}
     | otherwise                                 -> return $ Left $ "Read byte index out of range: " ++ show addr
...
-- Writes a byte (8bit)
writeByte :: Monad m => Word16 -> Word8 -> Z80State m Word8
writeByte addr val = return 0
